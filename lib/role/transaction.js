...
function getTransactionList(userId,companyId,minPrice,maxPrice,startTransactionDate,endTransactionDate,city,area,minServiceCharge,maxServiceCharge,minActualPrice,maxActualPrice,typeOfRental,isDelete,skip,limit,states,callback){
    ...
    if(utilsValue.isValid(startTransactionDate)&&utilsValue.isValid(endTransactionDate)){
        match.transactionDate = {
            $gte: getSaveDate(startTransactionDate),
...
function addTransaction(houseId,userId,companyId,actualPrice,serviceCharge,startRentDate,endRentDate,state,callback) {
...
function editTransaction(id,houseId,userId,companyId,actualPrice,serviceCharge,state,edit,transactionDate,startRentDate,endRentDate, callback) {
    if (id.length == 24){
        const searchDoc = {
            '_id': ObjectId(id)
        }
...
function getTransactionById(id,isDelete,callback){
    if(isDelete == 'true'){
        isDelete = true
    }
    if(isDelete == 'false'){
    ...
    mongoDB.queryFindOne(collectionName, searchDoc,callback)
}
